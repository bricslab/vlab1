...
padded_o_div = document.querySelector(".padded-o-div").style.paddingTop = "0";

function check2() {
  var score = 0;
  const q = [];
  for (let i = 1; i <= 5; i++) {
    q.push(document.querySelector(`input[name="pquestion${i}"]:checked`) && document.querySelector(`input[name="pquestion${i}"]:checked`).value);
  }

  for (let j = 1; j <= 4; j++) {
    for(let i = 1; i <= 5; i++) {
      document.getElementById(`pq${i}${j}`).style.color = "black";
    }
  }

  let ans = ['pq12', 'pq22', 'pq32', 'pq41', 'pq51'];
  ...
  for(let i = 0; i < 5; i++) {
    ...
    else if(q[i] && q[i] != ans[i]) {
      document.getElementById(q[i]).style.color = 'red';
    }
  }
  let msg = `${score} of 5`;
  document.getElementById("p_userscore").innerHTML = msg.toString();
}
...
function check() {
  ...
  const q = [];
  for (let i = 1; i <= 5; i++) {
    q.push(document.querySelector(`input[name="question${i}"]:checked`) && document.querySelector(`input[name="question${i}"]:checked`).value);
  }
  
  for (let j = 1;j <= 4; j++) {
    for(let i = 1; i <= 5; i++) {
      document.getElementById(`q${i}${j}`).style.color = "black";
    }
  }

  let ans = ['q11', 'q23', 'q32', 'q41', 'q51'];
  ...
  let msg = `${score} of 5`;
  document.getElementById("userscore").innerHTML = msg.toString();
}